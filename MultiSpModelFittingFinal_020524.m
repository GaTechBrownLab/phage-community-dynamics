...
%filename_coeffs = 'coefficients_011824.xlsx';
...
%% Interaction coefficients heat map

% scale beta values by beta_11 (PA-PA)
raw_beta = BetaMC(1:4,1:4);
sc_beta = raw_beta./raw_beta(1,1);

cmin = min(min(sc_beta));
cmax = max(max(sc_beta));
%clims = [cmin cmax];
clims = [0 2.5];
%redmap = flipud([ones(256,1),linspace(1,0,256)',linspace(1,0,256)']);
redmap = [ones(256,1),linspace(1,0,256)',linspace(1,0,256)'];

figcoeffs = figure;
set(gcf,'Unit','Inches','Position',[0,0,5,4]);
hold on
box on
imagesc(flipud(sc_beta),clims)
xticks([1,2,3,4])
xticklabels({'PA','AB','BC','SA'})
xlabel('Species j (actor)','FontWeight','normal','FontSize',9)
yticks([1,2,3,4])
yticklabels({'SA','BC','AB','PA'})
ylabel('Species i (recipient)','FontWeight','normal','FontSize',9)
xlim([0.5 4.5])
ylim([0.5 4.5])
colormap(redmap)
colorbar
text(0.9,4,num2str(sc_beta(1,1),'%d'),'FontSize',8); text(1.9,4,num2str(sc_beta(1,2),'%.2f'),'FontSize',8); text(2.9,4,num2str(sc_beta(1,3),'%.2f'),'FontSize',8); text(3.9,4,num2str(sc_beta(1,4),'%.2f'),'FontSize',8)
text(0.9,3,num2str(sc_beta(2,1),'%.2f'),'FontSize',8); text(1.9,3,num2str(sc_beta(2,2),'%.2f'),'FontSize',8); text(2.9,3,num2str(sc_beta(2,3),'%.2f'),'FontSize',8); text(3.9,3,num2str(sc_beta(2,4),'%.2f'),'FontSize',8)
text(0.9,2,num2str(sc_beta(3,1),'%.2f'),'FontSize',8); text(1.9,2,num2str(sc_beta(3,2),'%.2f'),'FontSize',8); text(2.9,2,num2str(sc_beta(3,3),'%.2f'),'FontSize',8); text(3.9,2,num2str(sc_beta(3,4),'%.2f'),'FontSize',8)
text(0.9,1,num2str(sc_beta(4,1),'%.2f'),'FontSize',8); text(1.9,1,num2str(sc_beta(4,2),'%.2f'),'FontSize',8); text(2.9,1,num2str(sc_beta(4,3),'%.2f'),'FontSize',8); text(3.9,1,num2str(sc_beta(4,4),'%.2f'),'FontSize',8)
title({'Interaction coefficients \beta_{ij} for PA14 Polyculture'},'FontWeight','normal','FontSize',10)

% Save figures
%exportgraphics(figcoeffs,'FigS8heatmaps_MatlabFinal3.eps','Resolution',300,'ContentType','vector')
